...
// Pano2VR 5.2.4/15996
// Filename: disclaimer.ggsk
// Generated vi. jul. 12 22:01:28 2019

function pano2vrSkin(player,base) {
	var ggSkinVars = [];
	ggSkinVars['ht_anim'] = false;
	var me=this;
	var flag=false;
	var nodeMarker=[];
	...
	this.addSkin=function() {
		...
		this._timer_1.ggActivate=function () {
			ggSkinVars['ht_anim'] = !ggSkinVars['ht_anim'];
		}
		...
		this._text_1.ggLeft=-247;
		this._text_1.ggTop=-135;
		...
		this._text_1.style[domTransform + 'Origin']='50% 100%';
		hs ='position:absolute;';
		hs+='left: 0px;';
		hs+='top:  0px;';
		hs+='width: 490px;';
		hs+='height: 56px;';
		hs+='border: 2px solid #e3e3e3;';
		hs+='border-radius: 5px;';
		hs+=cssPrefix + 'border-radius: 5px;';
		hs+='color: rgba(227,227,227,1);';
		hs+='text-align: center;';
		hs+='white-space: pre-wrap;';
		hs+='padding: 1px 2px 1px 2px;';
		hs+='overflow: hidden;';
		this._text_1__text.setAttribute('style',hs);
		this._text_1__text.innerHTML="Digital walk-through is not intended show exact finishes, material types or dimensions. Actual finishes will vary from those depicted in the digital walk-through.";
	...
	function SkinHotspotClass(skinObj,hotspot) {
		...
		} else
		{
			...
			this._htimage=document.createElement('div');
			this._htimage.ggId="HT-image";
			this._htimage.ggParameter={ rx:0,ry:0,a:0,sx:1,sy:1 };
			this._htimage.ggVisible=true;
			this._htimage.className='ggskin ggskin_container ';
			this._htimage.ggType='container';
			hs ='';
			hs+='height : 32px;';
			hs+='left : -15px;';
			hs+='position : absolute;';
			...
			hs+='width : 32px;';
			hs+='pointer-events:none;';
			this._htimage.setAttribute('style',hs);
			this._htimage.style[domTransform + 'Origin']='50% 50%';
			me._htimage.ggIsActive=function() {
				if ((this.parentNode) && (this.parentNode.ggIsActive)) {
					return this.parentNode.ggIsActive();
				}
				return false;
			}
			me._htimage.ggElementNodeId=function() {
				if ((this.parentNode) && (this.parentNode.ggElementNodeId)) {
					return this.parentNode.ggElementNodeId();
				}
				return me.ggNodeId;
			}
			this._htimage.ggUpdatePosition=function (useTransition) {
			}
			...
			this._htimage.appendChild(this._rectangle_1);
			this._rectangle2=document.createElement('div');
			this._rectangle2.ggId="Rectangle2";
			this._rectangle2.ggParameter={ rx:0,ry:0,a:0,sx:1,sy:1 };
			this._rectangle2.ggVisible=true;
			this._rectangle2.className='ggskin ggskin_rectangle ';
			this._rectangle2.ggType='rectangle';
			hs ='';
			hs+=cssPrefix + 'border-radius : 999px;';
			hs+='border-radius : 999px;';
			hs+='border : 2px solid #ffffff;';
			...
			hs+='width : 30px;';
			hs+='pointer-events:auto;';
			this._rectangle2.setAttribute('style',hs);
			this._rectangle2.style[domTransform + 'Origin']='50% 50%';
			me._rectangle2.ggIsActive=function() {
				if ((this.parentNode) && (this.parentNode.ggIsActive)) {
					return this.parentNode.ggIsActive();
				}
				return false;
			}
			me._rectangle2.ggElementNodeId=function() {
				if ((this.parentNode) && (this.parentNode.ggElementNodeId)) {
					return this.parentNode.ggElementNodeId();
				}
				return me.ggNodeId;
			}
			me._rectangle2.ggCurrentLogicStateScaling = -1;
			me._rectangle2.ggCurrentLogicStateAlpha = -1;
			this._rectangle2.ggUpdateConditionTimer=function () {
				var newLogicStateScaling;
				if (
					(ggSkinVars['ht_anim'] == true)
				)
				...
				if (me._rectangle2.ggCurrentLogicStateScaling != newLogicStateScaling) {
					me._rectangle2.ggCurrentLogicStateScaling = newLogicStateScaling;
					me._rectangle2.style[domTransition]='' + cssPrefix + 'transform 500ms ease 0ms, opacity 500ms ease 0ms, visibility 500ms ease 0ms';
					if (me._rectangle2.ggCurrentLogicStateScaling == 0) {
						me._rectangle2.ggParameter.sx = 0.5;
						me._rectangle2.ggParameter.sy = 0.5;
						me._rectangle2.style[domTransform]=parameterToTransform(me._rectangle2.ggParameter);
					}
					else {
						me._rectangle2.ggParameter.sx = 1;
						me._rectangle2.ggParameter.sy = 1;
						me._rectangle2.style[domTransform]=parameterToTransform(me._rectangle2.ggParameter);
					}
				}
				var newLogicStateAlpha;
				...
				if (me._rectangle2.ggCurrentLogicStateAlpha != newLogicStateAlpha) {
					me._rectangle2.ggCurrentLogicStateAlpha = newLogicStateAlpha;
					me._rectangle2.style[domTransition]='' + cssPrefix + 'transform 500ms ease 0ms, opacity 500ms ease 0ms, visibility 500ms ease 0ms';
					if (me._rectangle2.ggCurrentLogicStateAlpha == 0) {
						me._rectangle2.style.visibility=me._rectangle2.ggVisible?'inherit':'hidden';
						me._rectangle2.style.opacity=1;
					}
					else {
						me._rectangle2.style.visibility="hidden";
						me._rectangle2.style.opacity=0;
					}
				}
			}
			this._rectangle2.ggUpdatePosition=function (useTransition) {
			}
			this._htimage.appendChild(this._rectangle2);
			this.__div.appendChild(this._htimage);
			this.hotspotTimerEvent=function() {
				setTimeout(function() { me.hotspotTimerEvent(); }, 10);
				me._rectangle_1.ggUpdateConditionTimer();
				me._rectangle2.ggUpdateConditionTimer();
			}
